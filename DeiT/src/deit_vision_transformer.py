...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, qqkkvv = False, LN_affine = True):
        super().__init__()
        if norm_layer != None:
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, is_feat=False, quant_params=None):
        ...
        if is_feat and self.model_type == 'student':
            last_block = self.blocks[-1]
            if hasattr(last_block, 'mlp') and hasattr(last_block.mlp, 'fc1'):
                if hasattr(last_block.mlp.fc1, 'is_last_mlp_fc1') and last_block.mlp.fc1.is_last_mlp_fc1:
                    last_block.mlp.fc1.save_quantized_input = True
            ...
            if self.model_type == 'student':
                ...
                if hasattr(last_block, 'mlp') and hasattr(last_block.mlp, 'fc1'):
                    ...
                    if hasattr(fc1, 'saved_quantized_input') and fc1.saved_quantized_input is not None:
                        ...
                        fc1.save_quantized_input = False
                    else:
                        if len(intermediate_features) > 0:
                            feat = intermediate_features[-1]
                ...
                last_block = self.blocks[-1]
                if len(intermediate_features) > 0:
                    feat_t = intermediate_features[-1]  
                    if self.distill_token_mode == 'cls_only':
                        feat_t = feat_t[:, 0, :]  
                else:
                    feat_t = x 
                ...
                if self.QFeatureFlag and hasattr(self, 'feature_quantizer'):
                    ...
                    if self.distill_token_mode == 'cls_only' and fd_map.shape[1] == 1:
        ...
        if self.dist_token is None:
            if is_feat:
                return self.pre_logits(x[:, 0]), attn_matrixs, intermediate_features, fd_map
            return self.pre_logits(x[:, 0]), attn_matrixs, intermediate_features
        else:
            if is_feat:
                return x[:, 0], x[:, 1], attn_matrixs, intermediate_features, fd_map
