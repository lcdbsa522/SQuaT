...
def main():
    ...
    parser.add_argument('--quant_method', type=str, default='EWGS', choices=['EWGS'], help='training with different quantization methods')
    ...
    parser.add_argument('--vis_every', type=int, default=100, help='visualize quantization bins every N epochs')
    ...
    if args.quant_method == "EWGS" or args.baseline:
    ...
    if args.quant_method == "EWGS" or args.baseline:
        ...
        for m in model.modules():
            if isinstance(m, QConv):
                ...
                # EWGS params
                if m.quan_weight:
                    if hasattr(m, 'lW'): quant_params_s.append(m.lW)
                    if hasattr(m, 'uW'): quant_params_s.append(m.uW)
                if m.quan_act:
                    if hasattr(m, 'lA'): quant_params_s.append(m.lA)
                    if hasattr(m, 'uA'): quant_params_s.append(m.uA)
                if m.quan_act or m.quan_weight:
                    if hasattr(m, 'output_scale'): quant_params_s.append(m.output_scale)
        ...
        raise NotImplementedError(f"Not implement {args.quant_method}!")
    ...
    if args.distill:
        ...
        for m in model_t.modules():
            if isinstance(m, FeatureQuantizer):
                if hasattr(m, 'lF'): quant_params_t.append(m.lF)
                if hasattr(m, 'uF'): quant_params_t.append(m.uF)
                if hasattr(m, 'output_scale'): quant_params_t.append(m.output_scale)
    ...
    for ep in range(args.start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_loader):            
            ...
            if args.distill:
                ...
                if args.distill == "crdst":
                    ...
                    loss_fd = criterion_kd(fd_map_s, fd_map_t)
                    loss_total = args.kd_gamma * loss_cls +  args.kd_beta * loss_div + args.kd_gamma * loss_fd
                    ...
                    with open(os.path.join(args.log_dir,'loss.txt'), "a") as w:
                        w.write(f"{content}\n")
                if ep == args.start_epoch and i == 0:
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(test_loader):
                ...
                correct_classified += (predicted == labels).sum().item()
            test_acc = correct_classified/total*100
            print(f"Current epoch: [{ep}/{args.epochs}] \t Test accuracy: {test_acc:.2f}%")
