...
def get_student_quant_params(model, layer_idx=-1):

    quant_params = {
        'clip_val': None,
        'input_bits': None,
        'quantizer_type': 'SymQuantizer'
    }
    ...
    try:
        if hasattr(model, 'bert') and hasattr(model.bert, 'encoder'):
            ...
            if hasattr(encoder, 'layer') and len(encoder.layer) > 0:
                if layer_idx == -1:
                    target_layer = encoder.layer[-1]
                else:
                    target_layer = encoder.layer[layer_idx] if layer_idx < len(encoder.layer) else encoder.layer[-1]
                
                if hasattr(target_layer, 'intermediate'):
                    intermediate_module = target_layer.intermediate
                    dense_module = getattr(intermediate_module, 'dense', None)

                    if dense_module is not None and hasattr(dense_module, 'act_clip_val'):
                        quant_params['clip_val'] = dense_module.act_clip_val.clone()

                    if dense_module is not None and hasattr(dense_module, 'input_bits'):
                        quant_params['input_bits'] = dense_module.input_bits
                    elif hasattr(model, 'config') and hasattr(model.config, 'input_bits'):
                        quant_params['input_bits'] = model.config.input_bits
    except Exception as e:
        logger.warning(f"Failed to extract quant params: {e}")
        if quant_params['clip_val'] is None:
            quant_params['clip_val'] = torch.tensor([-2.5, 2.5])
        if quant_params['input_bits'] is None:
            quant_params['input_bits'] = 8
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, teacher_probs=None):
        ...
        attention_prob = st_attention_probs
...
class BertSelfOutput(nn.Module):
    def __init__(self, config, i):
        ...
        self.squat_enable = getattr(config, 'squat_enable', False)
        self.squat_layer = getattr(config, 'squat_layer', -1) 
...
class BertIntermediate(nn.Module):
    ...
    def get_last_q_input(self):
        return getattr(self.dense, 'last_q_input', None)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, teacher_probs=None):
        ...
        ffn_fc1_qinput = self.intermediate.get_last_q_input()
        layer_output = self.output(intermediate_output, attention_output)

        if ffn_fc1_qinput is None:
            ffn_fc1_qinput = None

        if isinstance(layer_value, (tuple, list)):
            layer_value = tuple(layer_value) + (ffn_fc1_qinput,)
        else:
            layer_value = (layer_value, ffn_fc1_qinput)
